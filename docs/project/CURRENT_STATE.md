...
- Firestore Security Rules restrict `users/{uid}` document trees to the signed-in
  owner.
- Dashboard read/write helpers use user-owned document paths and preserve
  compatibility with older water millilitre fields while saving water in litres.
...
- Dashboard water model aligned with accepted litre units.
